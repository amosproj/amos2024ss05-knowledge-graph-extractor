...
def test_relation_extraction_from_llm_output():
    ...
    '''
    # Act
    relation = graph_handler.extract_relation_from_llm_output(llm_response, ['Autonomous'], ['Conference'])
    # Assert
...
def test_component_connection_with_llm(mocker):
    ...
    '''
    mocker.patch('graph_creator.llm_handler.check_for_connecting_relation', return_value=llm_response)
    ...
    rate_limit = 1
    result_entity_and_relations = graph_handler.connect_with_llm(df_entities_and_relations, chunks, rate_limit)

    # Assert
    #calculate components of new knowledge graph
    entities = graph_handler.extract_entity_set(result_entity_and_relations)
    entities_dict, relations_list = graph_handler.index_entity_relation_table(result_entity_and_relations, entities)
