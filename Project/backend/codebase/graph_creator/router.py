...
@router.post("/upload/")
async def upload_pdf(file: UploadFile = File(...), graph_job_dao: GraphJobDAO = Depends()):
    """
...
async def read_graph_jobs(offset: int = 0, limit: int = 100, graph_job_dao: GraphJobDAO = Depends()):
    """
...
async def read_graph_job_by_id(graph_job_id: uuid.UUID, graph_job_dao: GraphJobDAO = Depends()):
    """
        ...
        """
    graph_job = await GraphJobDAO.get_graph_job_by_id(graph_job_dao, graph_job_id=graph_job_id)
...
async def read_graph_job_by_name(graph_job_name: str, graph_job_dao: GraphJobDAO = Depends()):
    """
    ...
    graph_job = await GraphJobDAO.get_graph_job_by_name(graph_job_dao, name=graph_job_name)
